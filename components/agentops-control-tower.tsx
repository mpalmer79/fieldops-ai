...
export function AgentOpsControlTower() {
  ...
  if (!snapshot) return <div className="agentops-loading"><RefreshCw/><strong>Loading AgentOps control tower</strong><span>Reading fleet health, release gates, and deployment state.</span>{error && <button onClick={() => void load()}>Retry</button>}</div>;
  ...
  return <section className="agentops">
    ...
    {error && <div className="agentops-alert error"><OctagonAlert/><span>{error}</span><button onClick={() => void load()}>Refresh state</button></div>}
    {notice && <div className="agentops-alert success"><CheckCircle2/><span>{notice}</span></div>}
