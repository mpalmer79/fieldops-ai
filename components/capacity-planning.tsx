...
export function CapacityPlanning() {
  ...
  return <section className="capacity-planning">
    ...
    <div className="capacity-primary-grid">
      <article className="capacity-card forecast-card"><div className="capacity-card-heading"><div><h3>Demand versus staffed capacity</h3><p>Select a day to inspect its appointment gap</p></div><Button variant="outline" onClick={() => void refreshForecast()} disabled={Boolean(busy)}><RefreshCw className={busy === "forecast" ? "spinning" : ""}/> {busy === "forecast" ? "Forecasting..." : "Refresh forecast"}</Button></div><div className="capacity-day-focus"><div><small>SELECTED DAY</small><strong>{shortDate(selectedDay?.date)}</strong></div><div><small>Expected</small><strong>{selectedDay?.expected ?? 0}</strong></div><div><small>Capacity</small><strong>{selectedDay?.capacity ?? 0}</strong></div><div className={(selectedDay?.gap ?? 0) > 0 ? "risk" : "safe"}><small>Appointment gap</small><strong>{selectedDay?.gap ?? 0}</strong></div></div><div className="capacity-day-rail" aria-label="Fourteen-day capacity risk selector">{snapshot.daily.map(day => { const risk = day.gap > 0; const severity = day.gap >= 4 ? "high" : risk ? "medium" : "low"; return <button key={day.date} className={`${severity} ${selectedDay?.date === day.date ? "selected" : ""}`} onClick={() => setSelectedDate(day.date)} aria-pressed={selectedDay?.date === day.date}><small>{shortDate(day.date)}</small><span><i style={{ height: `${Math.max(16, Math.min(100, day.expected / Math.max(snapshot.metrics.peakDemand, 1) * 100))}%` }}/><b style={{ height: `${Math.max(10, Math.min(100, day.capacity / Math.max(snapshot.metrics.peakDemand, 1) * 100))}%` }}/></span><strong>{risk ? `+${day.gap}` : "OK"}</strong></button>; })}</div><div className="forecast-chart" aria-label="Fourteen-day demand and capacity chart"><ResponsiveContainer width="100%" height="100%"><ComposedChart data={chartData} margin={{ top: 10, right: 16, left: -12, bottom: 0 }}><CartesianGrid stroke="#27323a" vertical={false}/><XAxis dataKey="label" tick={{ fontSize: 11, fill: "#78838e" }} axisLine={false} tickLine={false}/><YAxis tick={{ fontSize: 11, fill: "#78838e" }} axisLine={false} tickLine={false}/><Tooltip contentStyle={{ borderRadius: 2, border: "1px solid #344149", background: "#0d1519", color: "#dfe7e9", fontSize: 12 }} labelStyle={{ fontWeight: 800, marginBottom: 5 }}/><Legend wrapperStyle={{ fontSize: 11, paddingTop: 8 }}/><Area type="monotone" dataKey="upper" name="Upper demand bound" stroke="#c58c34" fill="#f7d89e" fillOpacity={0.14} strokeWidth={1}/><Line type="monotone" dataKey="expected" name="Expected demand" stroke="#e09645" strokeWidth={3} dot={false}/><Line type="monotone" dataKey="capacity" name="Staffed capacity" stroke="#56e5c2" strokeWidth={3} dot={false}/></ComposedChart></ResponsiveContainer></div><div className="forecast-boundary"><ShieldCheck/><span><strong>Decision evidence</strong>{snapshot.boundaries.model}. Source: {snapshot.boundaries.data}.</span></div></article>
