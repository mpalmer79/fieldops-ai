...
function CapacityMetric({ icon, label, value, detail, tone = "" }: { icon: React.ReactNode; label: string; value: string; detail: string; tone?: string }) {
  return <div className={`capacity-metric ${tone}`}><span>{icon}</span><div><small>{label}</small><strong>{value}</strong><p>{detail}</p></div></div>;
}
...
function ScenarioRange({ label, value, minimum, maximum, suffix, onChange }: { label: string; value: number; minimum: number; maximum: number; suffix: string; onChange: (value: number) => void }) {
  return <label className="scenario-range"><span><strong>{label}</strong><b>{value > 0 && minimum < 0 ? "+" : ""}{value}{suffix}</b></span><input type="range" min={minimum} max={maximum} step={1} value={value} onChange={event => onChange(Number(event.target.value))}/><small>{minimum}{suffix}<i/>{maximum > 0 ? "+" : ""}{maximum}{suffix}</small></label>;
}
...
export function CapacityPlanning() {
  ...
  if (!snapshot) return <div className="capacity-loading"><RefreshCw/><strong>Loading demand and capacity plan</strong><span>Retrieving forecast evidence, service demand, and workforce constraints.</span>{error && <button onClick={() => void load()}>Retry</button>}</div>;
  ...
  return <section className="capacity-planning">
    ...
    <div className="capacity-primary-grid">
      <article className="capacity-card forecast-card"><div className="capacity-card-heading"><div><h3>Demand versus staffed capacity</h3><p>Expected appointments, uncertainty ceiling, and service slots</p></div><Button variant="outline" onClick={() => void refreshForecast()} disabled={Boolean(busy)}><RefreshCw className={busy === "forecast" ? "spinning" : ""}/> {busy === "forecast" ? "Forecasting..." : "Run new forecast"}</Button></div><div className="forecast-chart" aria-label="Fourteen-day demand and capacity chart"><ResponsiveContainer width="100%" height="100%"><ComposedChart data={chartData} margin={{ top: 10, right: 16, left: -12, bottom: 0 }}><CartesianGrid stroke="#e9edf1" vertical={false}/><XAxis dataKey="label" tick={{ fontSize: 11, fill: "#78838e" }} axisLine={false} tickLine={false}/><YAxis tick={{ fontSize: 11, fill: "#78838e" }} axisLine={false} tickLine={false}/><Tooltip contentStyle={{ borderRadius: 9, border: "1px solid #dce2e7", fontSize: 12 }} labelStyle={{ fontWeight: 800, marginBottom: 5 }}/><Legend wrapperStyle={{ fontSize: 11, paddingTop: 8 }}/><Area type="monotone" dataKey="upper" name="Upper demand bound" stroke="#c58c34" fill="#f7d89e" fillOpacity={0.24} strokeWidth={1}/><Line type="monotone" dataKey="expected" name="Expected demand" stroke="#d0762d" strokeWidth={3} dot={false}/><Line type="monotone" dataKey="capacity" name="Staffed capacity" stroke="#4354d8" strokeWidth={3} dot={false}/></ComposedChart></ResponsiveContainer></div><div className="forecast-boundary"><ShieldCheck/><span><strong>Decision evidence</strong>{snapshot.boundaries.model}. Source: {snapshot.boundaries.data}.</span></div></article>
