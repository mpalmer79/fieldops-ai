...
export function RepairOrdersBoard() {
  ...
  return <div className="repair-orders-view">
    ...
    <section className="repair-order-layout">
      <div className="promise-rail">
        ...
        <div className="repair-order-list">
          {repairOrders.map(order => {
            ...
            return <button key={order.repairOrder} className={`repair-order-row ${order.status} ${selectedOrder.repairOrder === order.repairOrder ? "selected" : ""}`} onClick={() => setSelectedOrderId(order.repairOrder)} aria-pressed={selectedOrder.repairOrder === order.repairOrder}>
              <span className="ro-identity"><CarFront/><span><strong>{order.repairOrder}</strong><small>{order.vehicle}</small></span></span>
              <span className="ro-timeline" aria-label={`${progress}% through service process, promised at ${order.promised}`}><i className="ro-progress" style={{ width: `${progress}%` }}/><b className="ro-deadline" style={{ left: `${promise}%` }}/><em>{order.operation}</em></span>
      ...
      <aside className={`promise-inspector ${selectedOrder.status === "at-risk" ? "risk" : ""}`} aria-live="polite">
        ...
        <div className="promise-progress"><span>Service journey</span><div>{phaseLabels.map((label, index) => <div className={index < selectedOrder.phase ? "complete" : index === selectedOrder.phase ? "current" : ""} key={label}><i>{index < selectedOrder.phase ? <Check/> : index + 1}</i><small>{label}</small></div>)}</div></div>
