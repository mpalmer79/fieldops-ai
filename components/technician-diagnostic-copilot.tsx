...
export function TechnicianDiagnosticCopilot() {
  ...
  return <section className="diagnostic-copilot">
    ...
    <div className="diagnostic-lower-grid">
      ...
      <article className="diagnostic-card outcome-card"><div className="diagnostic-card-heading"><div><h3>Technician outcome</h3><p>Close the feedback loop for quality evaluation</p></div>{snapshot.outcome && <span className="outcome-complete"><Check/> recorded</span>}</div>{snapshot.outcome ? <div className="recorded-outcome"><CheckCircle2/><div><strong>{snapshot.outcome.first_time_fix ? "First-time fix confirmed" : "Return visit required"}</strong><p>{snapshot.outcome.notes}</p><small>{snapshot.outcome.duration_minutes} minutes · recorded {new Date(snapshot.outcome.created_at).toLocaleDateString()}</small></div></div> : accepted ? <div className="outcome-form"><div><label><span>Repair duration</span><input type="number" min={5} max={480} value={durationMinutes} onChange={event => setDurationMinutes(Number(event.target.value))}/></label><label className="first-fix"><input type="checkbox" checked={firstTimeFix} onChange={event => setFirstTimeFix(event.target.checked)}/><span>Fixed right the first time</span></label></div><label><span>Technician notes</span><textarea value={outcomeNotes} onChange={event => setOutcomeNotes(event.target.value)} maxLength={500}/></label><Button onClick={() => void resolve()} disabled={Boolean(busy)}><Wrench/> {busy === "resolve" ? "Recording..." : "Record repair outcome"}</Button></div> : <div className="diagnostic-empty"><Clock3/><strong>Awaiting technician decision</strong><span>Accept a verification path before recording the repair outcome.</span></div>}</article>
