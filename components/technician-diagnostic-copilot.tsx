...
export function TechnicianDiagnosticCopilot() {
  ...
  const [outcomeNotes, setOutcomeNotes] = useState("Verified fan-circuit fault, completed repair, and confirmed stable freezer airflow.");
  ...
  useEffect(() => {
    ...
    void Promise.resolve(context.registerTool({ name: "run_diagnostic_analysis", title: "Run diagnostic analysis", description: "Create a versioned, evidence-grounded diagnostic analysis for the current field-service case.", inputSchema: { type: "object", properties: { caseId: { type: "string" } }, required: ["caseId"], additionalProperties: false }, annotations: { readOnlyHint: false, untrustedContentHint: false }, execute: async (input: unknown) => analyze((input as { caseId: string }).caseId) }, { signal: lifecycle.signal })).catch(() => undefined);
  ...
  if (!snapshot) return <div className="diagnostic-loading"><RefreshCw/><strong>Loading technician diagnostic case</strong><span>Retrieving evidence, service history, inventory, and safety policy.</span>{error && <button onClick={() => void load()}>Retry</button>}</div>;
  ...
  return <section className="diagnostic-copilot">
    <div className="diagnostic-intro"><div><span className="eyebrow"><Stethoscope/> Evidence-grounded field support</span><h2>Technician diagnostic copilot</h2><p>Turn service symptoms into traceable verification paths with explicit safety boundaries.</p></div><div className="diagnostic-context"><span>{snapshot.boundaries.scenario}</span><strong>{snapshot.case.workOrderId} · {snapshot.case.city}</strong><small>{snapshot.case.technicianName} · {snapshot.case.appointmentWindow}</small></div></div>
    {error && <div className="diagnostic-alert error"><XCircle/><span>{error}</span><button onClick={() => void load()}>Refresh case</button></div>}
    {notice && <div className="diagnostic-alert success"><CheckCircle2/><span>{notice}</span></div>}
    ...
    <div className="diagnostic-case-grid">
      <article className="diagnostic-card case-card"><div className="diagnostic-card-heading"><div><h3>Service case</h3><p>{snapshot.case.applianceMake} {snapshot.case.applianceModel} · serial ending {snapshot.case.serialTail}</p></div><span className={`case-status ${snapshot.case.status.toLowerCase()}`}>{displayStatus(snapshot.case.status)}</span></div><div className="complaint"><small>Customer complaint</small><p>{snapshot.case.complaint}</p><code>{snapshot.case.symptomCode}</code></div><div className="case-meta"><div><span>Technician</span><strong>{snapshot.case.technicianName}</strong><small>{snapshot.case.technicianId}</small></div><div><span>Analysis</span><strong>{snapshot.run.modelVersion}</strong><small>{snapshot.run.id.slice(-12)}</small></div><div><span>Tool results</span><strong>{snapshot.run.toolCallCount} successful</strong><small>{snapshot.run.sourceCount} verified sources</small></div></div></article>
      <article className="diagnostic-card reasoning-card"><div className="diagnostic-card-heading"><div><h3>Ranked diagnostic paths</h3><p>Confidence is evidence-weighted, not a replacement for testing</p></div><Button variant="outline" onClick={() => void analyze()} disabled={Boolean(busy)}><RefreshCw className={busy === "analyze" ? "spinning" : ""}/> {busy === "analyze" ? "Analyzing..." : "Run new analysis"}</Button></div><div className="recommendation-list">{snapshot.recommendations.map(item => <button key={item.id} className={item.id === selected?.id ? "selected" : ""} onClick={() => setSelectedId(item.id)}><span className="recommendation-rank">{item.rank}</span><div><strong>{item.component}</strong><small>{item.sourceIds.length} sources · {displayStatus(item.safetyClass)}</small></div><span className="confidence-score">{item.confidence}%</span></button>)}</div></article>
    </div>
    {selected && <div className="diagnostic-detail-grid">
      <article className="diagnostic-card hypothesis-detail"><div className="diagnostic-card-heading"><div><h3>{selected.component}</h3><p>{selected.faultCode} · {selected.groundingScore}% grounded</p></div><span className={`recommendation-state ${selected.status.toLowerCase()}`}>{selected.status}</span></div><div className="hypothesis-copy"><section><small>Why this path</small><p>{selected.rationale}</p></section><section><small>Verification step</small><p>{selected.verificationStep}</p></section></div><div className={`safety-control ${selected.safetyClass.toLowerCase()}`}><ShieldAlert/><div><strong>{selected.safetyClass === "STANDARD" ? "Standard service controls" : selected.safetyClass === "ESCALATE" ? "Specialist escalation required" : "Power isolation required"}</strong><p>{selected.safetyClass === "STANDARD" ? "Follow the referenced service procedure." : selected.safetyClass === "ESCALATE" ? "Do not continue to board-level testing without qualified support." : "Disconnect power and verify isolation before panel access or protected testing."}</p></div>{selected.safetyClass !== "STANDARD" && <label><input type="checkbox" checked={safetyAcknowledged} onChange={event => setSafetyAcknowledged(event.target.checked)}/><span>I acknowledge the safety boundary</span></label>}</div><div className="diagnostic-actions"><Button onClick={() => void accept()} disabled={!canAccept || Boolean(busy) || (selected.safetyClass !== "STANDARD" && !safetyAcknowledged)}><ClipboardCheck/> {busy === "accept" ? "Recording..." : selected.status === "ACCEPTED" ? "Path accepted" : "Accept verification path"}</Button><Button variant="outline" onClick={() => void escalate()} disabled={caseClosed || Boolean(busy)}><AlertTriangle/> {busy === "escalate" ? "Escalating..." : "Escalate case"}</Button></div>{!caseClosed && <label className="escalation-input"><span>Escalation rationale</span><textarea value={escalationReason} onChange={event => setEscalationReason(event.target.value)} maxLength={300}/></label>}</article>
    ...
    <div className="diagnostic-lower-grid">
      ...
      <article className="diagnostic-card outcome-card"><div className="diagnostic-card-heading"><div><h3>Technician outcome</h3><p>Close the feedback loop for quality evaluation</p></div>{snapshot.outcome && <span className="outcome-complete"><Check/> recorded</span>}</div>{snapshot.outcome ? <div className="recorded-outcome"><CheckCircle2/><div><strong>{snapshot.outcome.first_time_fix ? "First-time fix confirmed" : "Follow-up service required"}</strong><p>{snapshot.outcome.notes}</p><small>{snapshot.outcome.duration_minutes} minutes · recorded {new Date(snapshot.outcome.created_at).toLocaleDateString()}</small></div></div> : accepted ? <div className="outcome-form"><div><label><span>Repair duration</span><input type="number" min={5} max={480} value={durationMinutes} onChange={event => setDurationMinutes(Number(event.target.value))}/></label><label className="first-fix"><input type="checkbox" checked={firstTimeFix} onChange={event => setFirstTimeFix(event.target.checked)}/><span>Resolved on first visit</span></label></div><label><span>Technician notes</span><textarea value={outcomeNotes} onChange={event => setOutcomeNotes(event.target.value)} maxLength={500}/></label><Button onClick={() => void resolve()} disabled={Boolean(busy)}><Wrench/> {busy === "resolve" ? "Recording..." : "Record repair outcome"}</Button></div> : <div className="diagnostic-empty"><Clock3/><strong>Awaiting technician decision</strong><span>Accept a verification path before recording the repair outcome.</span></div>}</article>
