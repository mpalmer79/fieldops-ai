...
export default function Home() {
  ...
  const specializedView = agentOpsView || diagnosticView || capacityView || benchmarkView || shopBoardView || repairOrdersView || techniciansView;
  const viewTitle = agentOpsView ? "AI operations" : diagnosticView ? "Vehicle diagnostics" : capacityView ? "Capacity planning" : benchmarkView ? "Simulation lab" : shopBoardView ? "Shop board" : repairOrdersView ? "Repair orders" : techniciansView ? "Technicians" : "Service command";
  const viewDescription = agentOpsView ? "Agent fleet governance and release safety" : diagnosticView ? "Evidence, safety, parts, and repair outcomes" : capacityView ? "Appointment forecasts, staffing risk, and scenario decisions" : benchmarkView ? "Dealer-group scale, regression gates, and benchmark evidence" : shopBoardView ? "Live bay occupancy, work state, and customer promise risk" : repairOrdersView ? "Customer promise risk, repair progression, and recovery status" : techniciansView ? "Workload, certification coverage, and qualified capacity" : "Rooftop 01 · live service operation";
  const operatingMode = agentOpsView ? "Monitored fleet" : diagnosticView ? "Grounded support" : capacityView ? "Planning horizon" : benchmarkView ? "Measured runtime" : shopBoardView ? "Floor control" : repairOrdersView ? "Promise control" : techniciansView ? "Team control" : "Live operation";
