...
export default function Home() {
  ...
  </aside><section className="workspace"><header className="topbar"><button className="menu-button" onClick={() => setMobileNav(true)} aria-label="Open navigation"><Menu/></button><div><h1>{agentOpsView ? "AI operations" : diagnosticView ? "Field diagnostics" : capacityView ? "Capacity planning" : benchmarkView ? "Simulation lab" : "Dispatch command"}</h1><p>{agentOpsView ? "Agent fleet governance and release safety" : diagnosticView ? "Evidence, safety, parts, and service outcomes" : capacityView ? "Demand forecasts, staffing risk, and scenario decisions" : benchmarkView ? "Enterprise scale, regression gates, and benchmark evidence" : "Greater Boston service territory"}</p></div><div className="topbar-actions"><div className="operating"><CircleDot/> {agentOpsView ? "Monitored fleet" : diagnosticView ? "Grounded support" : capacityView ? "Planning horizon" : benchmarkView ? "Measured runtime" : "Live operation"} <ChevronDown/></div><span className="date">Saturday, Sep 12</span>{!specializedView && <><Button variant="outline" onClick={() => setPolicyOpen(true)} className="policy-button" disabled={busy || !backendOnline}><Settings2/> Policy controls</Button><Button onClick={() => void simulate()} className="simulate" disabled={busy || !backendOnline}><Sparkles/> {busy ? "Working..." : "Simulate disruption"}</Button></>}</div></header>
