...
import { Activity, AlertTriangle, ArrowRight, BrainCircuit, Check, ChevronDown, CircleDot, Gauge, Map, Menu, MoreHorizontal, Navigation, Route, Search, Settings2, ShieldCheck, Sparkles, Stethoscope, Undo2, Users, X } from "lucide-react";
...
const navItems = [[Gauge, "Command"], [Map, "Live map"], [Route, "Routes"], [Users, "Technicians"], [Activity, "Performance"], [Stethoscope, "Diagnostic Copilot"], [BrainCircuit, "AgentOps"]] as const;
...
export default function Home() {
  ...
  const specializedView = agentOpsView || diagnosticView;
  ...
  </aside><section className="workspace"><header className="topbar"><button className="menu-button" onClick={() => setMobileNav(true)} aria-label="Open navigation"><Menu/></button><div><h1>{agentOpsView ? "AI operations" : diagnosticView ? "Field diagnostics" : "Dispatch command"}</h1><p>{agentOpsView ? "Agent fleet governance and release safety" : diagnosticView ? "Evidence, safety, parts, and service outcomes" : "Greater Boston service territory"}</p></div><div className="topbar-actions"><div className="operating"><CircleDot/> {agentOpsView ? "Monitored fleet" : diagnosticView ? "Grounded support" : "Live operation"} <ChevronDown/></div><span className="date">Friday, Sep 12</span>{!specializedView && <><Button variant="outline" onClick={() => setPolicyOpen(true)} className="policy-button" disabled={busy || !backendOnline}><Settings2/> Policy controls</Button><Button onClick={() => void simulate()} className="simulate" disabled={busy || !backendOnline}><Sparkles/> {busy ? "Working..." : "Simulate disruption"}</Button></>}</div></header>
  {agentOpsView ? <div className="content agentops-content"><AgentOpsControlTower/></div> : diagnosticView ? <div className="content diagnostic-content"><TechnicianDiagnosticCopilot/></div> : <div className="content">{error && <div className="error-banner"><AlertTriangle/><span>{error}</span><button onClick={() => void loadSnapshot()}>Retry</button></div>}{notice && <div className="success-banner"><Check/><span>{notice}</span>{activePlan?.status === "EXECUTED" && snapshot?.operator.role === "admin" && <button onClick={() => void rollback()} disabled={busy}><Undo2/> Roll back execution</button>}</div>}
