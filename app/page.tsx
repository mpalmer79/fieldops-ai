...
const navItems = [[Gauge, "Command"], [Map, "Live map"], [Route, "Routes"], [Users, "Technicians"], [Activity, "Performance"], [CalendarRange, "Capacity Planning"], [FlaskConical, "Simulation Lab"], [Stethoscope, "Diagnostic Copilot"], [BrainCircuit, "AgentOps"]] as const;
...
export default function Home() {
  ...
  const specializedView = agentOpsView || diagnosticView || capacityView || benchmarkView;
  const viewTitle = agentOpsView ? "AI operations" : diagnosticView ? "Field diagnostics" : capacityView ? "Capacity planning" : benchmarkView ? "Simulation lab" : "Dispatch command";
  const viewDescription = agentOpsView ? "Agent fleet governance and release safety" : diagnosticView ? "Evidence, safety, parts, and service outcomes" : capacityView ? "Demand forecasts, staffing risk, and scenario decisions" : benchmarkView ? "Enterprise scale, regression gates, and benchmark evidence" : "Greater Boston service territory";
  const operatingMode = agentOpsView ? "Monitored fleet" : diagnosticView ? "Grounded support" : capacityView ? "Planning horizon" : benchmarkView ? "Measured runtime" : "Live operation";
  return <main className="app-shell"><aside className={`sidebar ${mobileNav ? "sidebar-open" : ""}`}>
    ...
    <div className="territory-code"><span>OPS CONTROL</span><strong>NE-01 · BOSTON</strong><small>42.3601° N / 71.0589° W</small></div>
    <nav aria-label="Main navigation"><p>Mission control</p>{navItems.map(([Icon, label], index) => <button key={label} onClick={() => { setSelected(label); setMobileNav(false); }} className={selected === label ? "active" : ""}><span className="nav-index">{String(index + 1).padStart(2, "0")}</span><Icon/><span>{label}</span>{label === "Routes" && <small>25</small>}</button>)}</nav>
...
function DecisionPanel({ audit, activePlan, onReview }: { audit: AuditRow[]; activePlan: PersistedPlan | null; onReview: () => void }) {
  ...
  return <aside className="panel decision-panel"><div className="panel-heading"><div><span className="panel-code">02 / DECISIONS</span><h2>Priority queue</h2><p>Immutable operational audit</p></div><button aria-label="Decision options"><MoreHorizontal/></button></div><div className="decision-list">{reviewable && <Decision icon={<AlertTriangle/>} title="Capacity disruption" time="Now" text="Technician T-274 is unavailable. Seven stops were evaluated for recovery." action={onReview}/>} {audit.slice(0, reviewable ? 3 : 4).map((row, index) => { const copy = auditCopy(row); return <Decision key={row.id} tone={index % 3 === 0 ? "purple" : index % 3 === 1 ? "green" : "gold"} icon={row.action === "POLICY_UPDATED" ? <Settings2/> : row.action.includes("ROLL") ? <Undo2/> : <Route/>} {...copy}/>; })}{audit.length === 0 && <Decision tone="green" icon={<ShieldCheck/>} title="Audit ready" time="Now" text="Persistent decision history will appear after the first operational action."/>}</div><button className="view-log">PostgreSQL audit channel <ShieldCheck/></button></aside>;
}
