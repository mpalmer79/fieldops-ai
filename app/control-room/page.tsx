...
export default function ControlRoomPage() {
  ...
  const techniciansView = selected === "Technicians";
  const performanceView = selected === "Performance";
  const specializedView = agentOpsView || diagnosticView || capacityView || benchmarkView || shopBoardView || repairOrdersView || techniciansView || performanceView;
  const viewTitle = agentOpsView ? "AI operations" : diagnosticView ? "Vehicle diagnostics" : capacityView ? "Capacity planning" : benchmarkView ? "Simulation lab" : shopBoardView ? "Shop board" : repairOrdersView ? "Repair orders" : techniciansView ? "Technicians" : performanceView ? "Performance" : "Service command";
  const viewDescription = agentOpsView ? "Agent fleet governance and release safety" : diagnosticView ? "Evidence, safety, parts, and repair outcomes" : capacityView ? "Appointment forecasts, staffing risk, and scenario decisions" : benchmarkView ? "Dealership recovery stress tests and release readiness" : shopBoardView ? "Live bay occupancy, work state, and customer promise risk" : repairOrdersView ? "Customer promise risk, repair progression, and recovery status" : techniciansView ? "Workload, certification coverage, and qualified capacity" : performanceView ? "Customer promises, shop output, and recovery impact" : "Rooftop 01 · live service operation";
  const operatingMode = agentOpsView ? "Monitored fleet" : diagnosticView ? "Grounded support" : capacityView ? "Planning horizon" : benchmarkView ? "Measured runtime" : shopBoardView ? "Floor control" : repairOrdersView ? "Promise control" : techniciansView ? "Team control" : performanceView ? "Outcome control" : "Live operation";
  return <main className="app-shell"><aside className={`sidebar ${mobileNav ? "sidebar-open" : ""}`}>
    <Link href="/" className="brand"><span className="brand-mark"><Route size={18}/></span><span className="brand-name">FIELD<span>/OPS</span><small>Autonomy console</small></span><span className="brand-index">AI</span></Link><button className="mobile-close" aria-label="Close navigation" onClick={() => setMobileNav(false)}><X/></button>
    ...
    <nav aria-label="Main navigation"><p>Service operation</p>{navItems.map(([Icon, label], index) => <button key={label} onClick={() => { setSelected(label); setMobileNav(false); window.scrollTo({ top: 0 }); }} className={selected === label ? "active" : ""}><span className="nav-index">{String(index + 1).padStart(2, "0")}</span><Icon/><span>{label}</span>{label === "Repair orders" && <small>25</small>}</button>)}</nav>
