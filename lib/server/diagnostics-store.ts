...
export async function ensureDiagnosticState(operator: Operator) {
  ...
  const statements: D1PreparedStatement[] = [
    ...
    insertToolCall(database, SEED_RUN_ID, operator, timestamp, "parts_lookup", { partCodes: recommendations.map(item => item.partCode) }, { availableUnits: 5, constrainedParts: ["STRTR-G70-25T"] }),
...
export async function analyzeDiagnosticCase(operator: Operator, input: { caseId: string; expectedVersion: number }) {
  ...
  const statements: D1PreparedStatement[] = [
    ...
    insertToolCall(database, runId, operator, timestamp, "parts_lookup", { partCodes: recommendations.map(item => item.partCode) }, { availableUnits: 5, constrainedParts: ["STRTR-G70-25T"] }, true),
